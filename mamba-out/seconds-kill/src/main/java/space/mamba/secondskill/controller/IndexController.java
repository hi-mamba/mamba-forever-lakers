package space.mamba.secondskill.controller;
...
import space.mamba.secondskill.common.Limit.RedisLimit;
...
public class IndexController {
    ...
    public String initDBAndRedisBefore(HttpServletRequest request) {
        ...
        try {
            ...
            // 重置缓存
            stringRedisTemplate.opsForValue().set(RedisKeysConstant.STOCK_COUNT + 1, "50");
            stringRedisTemplate.opsForValue().set(RedisKeysConstant.STOCK_SALE + 1, "0");
            stringRedisTemplate.opsForValue().set(RedisKeysConstant.STOCK_VERSION + 1, "0");
        } catch (Exception e) {
    ...
    public String createOptimisticLimitOrder(HttpServletRequest request, int sid) {
        int res = 0;
        try {
            if (RedisLimit.limit(stringRedisTemplate)) {
                res = orderService.createOptimisticOrder(sid);
            }
        } catch (Exception e) {
            log.error("Exception: " + e);
